...
export class StatsManager {
  ...
  finishRun() {
    ...
    const duration = this.runStats.finishedAt - this.runStats.startedAt;
  ...
  recordBet(value: number) {
    ...
    this.runStats.betUsage[value] = (this.runStats.betUsage[value] || 0) + 1;
  ...
  recordWin(amount: number) {
    ...
    console.log("recordWin", amount);

    this.runStats.highestWin = Math.max(this.runStats.highestWin, amount);

    // PLAYER

    this.playerStats.totalWon += amount;

    this.playerStats.highestWin = Math.max(this.playerStats.highestWin, amount);
  }
  ...
  recordWinStreak(streak: number) {
    ...
    console.log("recordWinStreak", streak);

    this.runStats.bestWinStreak = Math.max(this.runStats.bestWinStreak, streak);
  ...
  recordLoseStreak(streak: number) {
    ...
    this.runStats.biggestLoseStreak = Math.max(
      this.runStats.biggestLoseStreak,
      streak,
    );

    this.playerStats.biggestLoseStreak = Math.max(
  ...
  getWinRateAllTime() {
    if (this.playerStats.runs === 0) return 0;

    return this.playerStats.runsWon / this.playerStats.runs;
  }

  getAccuracyAllTime() {
    const total = this.playerStats.totalBets;

    if (total === 0) return 0;

    return Math.round((this.playerStats.successfulBets / total) * 100);
  }

  getAverageWinAllTime() {
    if (this.playerStats.runsWon === 0) return 0;

    return this.playerStats.totalWon / this.playerStats.runsWon;
  }

  getAverageBetValueAllTime() {
    if (this.playerStats.totalBets === 0) return 0;

    return this.playerStats.totalBetValue / this.playerStats.totalBets;
  }

  getFavoriteBetCurrentRun() {
    return this.formatBet(this.getMostUsed(this.runStats.betUsage));
  }

  getFavoriteBetAllTime() {
    return this.formatBet(this.getMostUsed(this.playerStats.betUsage));
  }

  getFavoriteCombinationCurrentRun() {
    return this.getMostUsed(this.runStats.combinationUsage);
  }

  getLuckiestCombinationCurrentRun() {
    return this.getMostUsed(this.runStats.winningCombinationUsage);
  }

  getAccuracyCurrentRun() {
    if (this.runStats.totalBets === 0) return 0;

    return Math.round(
      (this.runStats.successfulBets / this.runStats.totalBets) * 100,
    );
  }
  ...
  getAverageBetValueCurrentRun() {
    ...
    return this.runStats.totalBetValue / this.runStats.totalBets;
  }

  getFavoriteCombinationAllTime() {
    return this.getMostUsed(this.playerStats.combinationUsage);
  }

  getLuckiestCombinationAllTime() {
    return this.getMostUsed(this.playerStats.winningCombinationUsage);
  }

  getRunDuration() {
    return Date.now() - this.runStats.startedAt;
  }

  getFormattedRunDuration() {
    const ms =
      (this.runStats.finishedAt ?? Date.now()) - this.runStats.startedAt;
  ...
  getFormattedTotalPlayTime(): string {
    const totalSeconds = Math.floor(this.playerStats.totalPlayTime / 1000);
  ...
  getAddictionRank(): TranslationKey {
    const hours = this.playerStats.totalPlayTime / 1000 / 60 / 60;

    if (hours < 1) return "curiousVisitor";

    if (hours < 5) return "coinFlipper";

    if (hours < 15) return "luckyBeginner";

    if (hours < 35) return "regularGambler";

    if (hours < 75) return "highRoller";

    if (hours < 150) return "casinoVeteran";

    if (hours < 300) return "coinMaster";

    if (hours < 500) return "casinoLegend";
  ...
  private getMostUsed(data: Record<string, number>): string | null {
  ...
  getCombinationUsageCurrentRun() {
    return Object.entries(this.runStats.combinationUsage).sort(
      (a, b) => b[1] - a[1],
    );
  }
