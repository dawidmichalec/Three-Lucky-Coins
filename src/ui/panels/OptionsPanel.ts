...
export class OptionsPanel extends Container {
  ...
  constructor(
    ...
    this.audioManager = AudioManager.getInstance(this.settingsManager);
  ...
  private createTitle(width: number) {
    ...
    title.position.set(width / 2, 108);
  ...
  private createMusicSlider() {
    this.musicSlider = new Slider({
      initialValue: this.settingsManager.get().musicVolume,
    ...
    this.musicSlider.position.set(1235.7, 350.1);
  ...
  private createSfxSlider() {
    this.sfxSlider = new Slider({
      initialValue: this.settingsManager.get().sfxVolume,
    ...
    this.sfxSlider.position.set(1235.7, 440.3);
  ...
  private createBrightnessSlider() {
    const slider = new Slider({
      initialValue: this.settingsManager.get().brightness,

      onChange: (value) => {
        this.settingsManager.setBrightness(value);

        this.displayManager.setBrightness(value);
      },
    });

    slider.position.set(1235.7, 530.3);
  ...
  private createLabelsForOptions() {
    const audioLabel = new LocalizedText("audio", {
      fill: 0xffffff,
      font: "Open Sans",
      fontSize: 32,
      fontWeight: "bold",
    ...
    const musicLabel = new LocalizedText("music", {
      fill: 0xffffff,
      font: "Open Sans",
      fontSize: 32,
      fontWeight: "bold",
    ...
    const soundEffectsLabel = new LocalizedText("soundEffects", {
      fill: 0xffffff,
      font: "Open Sans",
      fontSize: 32,
      fontWeight: "bold",
    ...
    const brightnessLabel = new LocalizedText("brightness", {
      fill: 0xffffff,
      font: "Open Sans",
      fontSize: 32,
      fontWeight: "bold",
    ...
    const fullScreenLabel = new LocalizedText("fullscreen", {
      fill: 0xffffff,
      font: "Open Sans",
      fontSize: 32,
      fontWeight: "bold",
    ...
    const languageLabel = new LocalizedText("language", {
      fill: 0xffffff,
      font: "Open Sans",
      fontSize: 32,
      fontWeight: "bold",
  ...
  createFullScreenToggleButton() {
    ...
    this.displayManager.setFullscreenListener((enabled) => {
  ...
  private updateLanguageLabel() {
    this.languageLabel.text = LANGUAGE_CONFIG[this.currentLanguageIndex].name;
  }

  private createLanguageSelector() {
    this.languageLabel = new Text({
      text: LANGUAGE_CONFIG[this.currentLanguageIndex].name,
    ...
    const left = new TriangleButton({
      ...
      onClick: () => {
        ...
        if (this.currentLanguageIndex < 0) {
          this.currentLanguageIndex = LANGUAGE_CONFIG.length - 1;
        }
  ...
  private createSaveButton() {
    this.saveButton = new RoundedButton({
      ...
      onClick: () => {
        const selectedLanguage = LANGUAGE_CONFIG[this.currentLanguageIndex].id;

        this.settingsManager.setLanguage(selectedLanguage);
  ...
  private syncAudioState() {
    const enabled = this.settingsManager.get().audioEnabled;
  ...
  show() {
    const currentLanguage = this.settingsManager.get().language;

    this.currentLanguageIndex = LANGUAGE_CONFIG.findIndex(
