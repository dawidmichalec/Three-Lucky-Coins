...
export class CollectionsPanel extends Container {
  ...
  createButtons() {
    ...
    this.addChild(this.playerButton, this.dealersButton, this.perksButton);
  }
  ...
  private createCollectionDetailPanel() {
    this.collectionDetailPanel = new CollectionDetailPanel(() => {
      this.collectionDetailPanel.hide();
    });

    this.collectionDetailPanel.zIndex = 5000;

    this.addChild(this.collectionDetailPanel);
  }
  ...
  private createLockedDealerTooltip() {
    ...
    this.addChild(this.lockedDealerTooltip);
  }

  private showLockedDealerTooltip() {
    if (this.lockedDealerTooltipTimeout) {
      clearTimeout(this.lockedDealerTooltipTimeout);
    }
  ...
  private async createDealerCollection(): Promise<void> {
    const content = new DealerCollectionContent({
      ...
      onDealerClick: (dealer) => {
        this.handleDealerClick(dealer);
      },
    });

    await content.init();

    this.scrollableContainer.addContent(content);
  }

  private handleDealerClick(dealer: DealerData) {
    const isDiscovered = this.dealerCollectionManager.isDealerDiscovered(
      dealer.id,
    );
    ...
    void this.collectionDetailPanel.showDealer(dealer);
  }

  private createPerkCollectionPlaceholder() {
    const placeholder = new LocalizedText("perks", {
    ...
    this.scrollableContainer.addContent(placeholder);
  }
