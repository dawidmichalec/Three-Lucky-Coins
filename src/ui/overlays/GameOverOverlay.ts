...
export class GameOverOverlay extends Container {
  ...
  constructor(
    ...
    this.gameOverLabel = new LocalizedText("gameOver", {
      /*
    ...
    this.addChild(this.background, this.gameOverLabel);
  }

  async play(visibleDuration = 1300): Promise<void> {
    if (this.isAnimating) {
      return;
    }
    ...
    this.gameOverLabel.scale.set(0.88);
  ...
  private fadeIn(duration: number): Promise<void> {
    return new Promise((resolve) => {
      ...
      const animate = (currentTime: number) => {
        ...
        this.gameOverLabel.scale.set(scale);
