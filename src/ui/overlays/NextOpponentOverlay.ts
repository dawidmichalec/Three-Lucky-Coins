...
export class NextOpponentOverlay extends Container {
  ...
  constructor(
    ...
    const nextOpponentLabel = new LocalizedText("nextOpponent", {
    ...
    nextOpponentLabel.position.set(layout.DESIGN_WIDTH / 2, 80);
  ...
  private createScrollContent(dealer: DealerData) {
    ...
    currentY = nameLabel.y + nameLabel.height + 6;

    const titleLabel = new LocalizedText(dealer.title, {
      font: "Open Sans",
      fontWeight: "bold",
      fontSize: 24,
      fill: 0xffd21f,
    ...
    if (dealer.skills.length === 0) {
      ...
      noSkillsText.position.set(0, currentY);

      this.scrollContent.addChild(noSkillsText);
  ...
  show() {
    if (this.fadeAnimationId !== undefined) {
