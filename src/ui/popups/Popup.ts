...
export class Popup extends Container {
  constructor({ message, width, height, onClose }: PopupProps) {
    ...
    // BACKGROUND
    const bg = new Graphics().roundRect(0, 0, width, height, 50).fill(0x4ca626);

    this.addChild(bg);

    // TEXT
    const text = new LocalizedText(message, {
      font: "Open Sans",
      fontSize: 24,
      fontWeight: "bold",
      fill: 0xffffff,
    ...
    // BUTTON OK
    const button = new Graphics().roundRect(0, 0, 120, 40, 10).fill(0x4ca626);
    ...
    const buttonText = new LocalizedText("popUpButtonText", {
      font: "Open Sans",
      fontSize: 24,
      fill: 0xffffff,
      fontWeight: "bold",
