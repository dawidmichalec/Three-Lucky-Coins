...
export class Slider extends Container {
  ...
  constructor(private options: SliderOptions) {
    ...
    this.sliderWidth = options.width ?? 300;

    this.sliderHeight = options.height ?? 12;
  ...
  private setupInteraction() {
    ...
    this.on("pointerdown", (event) => {
      this.updateFromPointer(event.global.x);
    });
  ...
  private updateFromPointer(globalX: number) {
    ...
    let value = local.x / this.sliderWidth;
